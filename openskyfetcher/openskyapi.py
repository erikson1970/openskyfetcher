...
class OpenSkyApi(object):
    ...
    """
    def __init__(self, username=None, password=None, proxyDict=None, verify=True):
        ...
        :param proxyDict: proxy instructions (optional)
        :param verify: SSL cert verification instructions (optional)
        """
    ...
    def _get_json(self, url_post, callee, params=None):
        r = requests.get("{0:s}{1:s}".format(self._api_url, url_post),
                         ...
                         proxies=self._local_proxy, verify=self._ssl_verify)# if self._local_proxy is not None else requests.get("{0:s}{1:s}".format(self._api_url, url_post),auth=self._auth, params=params, timeout=15.00)
